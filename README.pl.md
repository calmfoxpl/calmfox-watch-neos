...
z hubem, co wtyczka WordPressa oraz pakiety dla Magento i Syliusa (ten sam
kształt odpowiedzi, ten sam podpis, ta sama droga parowania): wystawia sekretny
adres kontrolny, który hub odpytuje w modelu pull.
...
`composer require`. Composer uruchamia wtedy instalator Neosa, przelicza
autoloader i publikuje zasoby, czyli robi to samo, co przy pakiecie pobranym
z Packagista:
...
CIEMNYM (tym samym, który panel Calmfox Watch pokazuje w motywie ciemnym), bo panel
Neosa jest ciemny, a panele WordPressa, Magento i Syliusa są białe i biorą wariant jasny. Odcienie stoją w tym samym miejscu koła
...
Testy mają własny autoloader (`tests/bootstrap.php`), więc wystarczy dowolny
PHPUnit 10 lub nowszy, bez `composer install`:
...
a ręcznie wpisane pole zawsze prędzej czy później się rozjeżdża). Skrypt, który
buduje paczkę `calmfox-watch-neos.zip`, czyta tę samą stałą.
